...
class CustomMultiLayerDCNV2(keras.layers.Layer):
    """
    ...
    """
    def __init__(self, num_layers, low_rank_dim, **kwargs):
    ...
    def build(self, input_shape):
        ...
        for _ in range(self.num_layers):
            self._v_layers.append(
                keras.layers.Dense(self.low_rank_dim, use_bias=False, name=f"v_layer_{_}")
            )
            self._u_layers.append(
                keras.layers.Dense(dim, use_bias=False, name=f"u_layer_{_}")
            )
            self._biases.append(self.add_weight(shape=(dim,), initializer="zeros", name=f"bias_{_}"))
        super().build(input_shape)

    def call(self, x0):
        x = x0
        for i in range(self.num_layers):
            x_proj_v = self._v_layers[i](x)
            x_proj_u = self._u_layers[i](x_proj_v)

            # Core DCN-V2 logic: x_{i+1} = x_0 * (U*V^T*x_i + b) + x_i
            x = x0 * (x_proj_u + self._biases[i]) + x
        return x
...
def create_dlrm_model(embedding_layers):
    """Creates the full DLRM model with the DCN-V2 cross network."""
    ...
    interaction_output = CustomMultiLayerDCNV2(
        num_layers=3,
        low_rank_dim=DCN_LOW_RANK_DIM
    )(all_features)
